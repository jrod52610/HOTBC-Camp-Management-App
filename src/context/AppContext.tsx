...
interface AppContextType {
  ...
  inviteUserBySMS: (phoneNumber: string, name: string, permissions: Permission[]) => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Real SMS invitation function using Twilio API
  const inviteUserBySMS = async (phoneNumber: string, name: string, permissions: Permission[] = ['read-only']) => {
    // Format phone number (remove non-numeric characters)
    const formattedPhoneNumber = phoneNumber.replace(/\D/g, '');
    
    // Add country code if not present
    const fullPhoneNumber = formattedPhoneNumber.startsWith('1') 
    ...
    try {
      // Prepare the SMS message with the invitation
      const smsMessage = `Welcome to HOTBC Management! You've been invited by an admin. Your temporary password is: ${tempPassword}`;
      
      // Send SMS via the API
      const { sendSms } = await import('@/lib/api/twilio');
      ...
      if (result.success) {
        // SMS was sent successfully, update user records
        if (existingUser) {
          // Update existing user's invitation status
          setUsers(prev => prev.map(user => 
          ...
          // Create a new user with sent invitation
          const newUser = {
            id: uuidv4(),
            name,
        ...
        console.log(`SMS invitation sent to ${fullPhoneNumber} for user ${name} with temporary password: ${tempPassword}`);
        return true;
      } else {
        // SMS failed to send
        console.error('Failed to send SMS invitation:', result.error);
        return false;
      }
    } catch (error) {
  ...
  const resendInvitation = async (userId: string) => {
    ...
    if (!user || !user.phoneNumber) {
      console.error('Cannot resend invitation: User not found or missing phone number');
      return false;
    }
    ...
    try {
      // Prepare the SMS message with the invitation
      const smsMessage = `HOTBC Management: Your invitation has been resent. Your new temporary password is: ${tempPassword}`;
      ...
      if (result.success) {
        ...
        console.log(`SMS invitation resent to ${fullPhoneNumber} for user ${user.name} with new temporary password: ${tempPassword}`);
        return true;
      } else {
        // SMS failed to send
        console.error('Failed to resend SMS invitation:', result.error);
        return false;
      }
    } catch (error) {
      console.error('Error resending SMS invitation:', error);
      return false;
    }
  };

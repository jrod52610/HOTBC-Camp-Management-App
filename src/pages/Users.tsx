...
export default function UsersPage() {
  ...
  const handleSendInvitation = async () => {
    if (inviteUser.name && inviteUser.phoneNumber) {
      ...
      try {
        const success = await inviteUserBySMS(phoneNumber, inviteUser.name, permissions);
        
        if (success) {
          // Show success toast
          toast.success(`Invitation sent to ${inviteUser.name} via SMS`);
          ...
          toast.error('Failed to send SMS invitation. Please try again.');
        }
      } catch (error) {
  ...
  const handleResendInvitation = async (userId: string) => {
    setResendingIds(prev => [...prev, userId]);
    
    try {
      const success = await resendInvitation(userId);
      if (success) {
      ...
      setResendingIds(prev => prev.filter(id => id !== userId));
    }
  };
            ...
            <Dialog>
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  {isSendingInvite ? (
                    <Button disabled>Sending Invitation...</Button>
                  ) : (
                    <DialogClose asChild>
                  ...
                  <CardContent className="p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <Button 
                        ...
                        disabled={!user.phoneNumber || resendingIds.includes(user.id)}
                      >
                        {resendingIds.includes(user.id) ? (
                          <>Sending...</>
                        ) : (
                          <>
